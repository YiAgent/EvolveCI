...
CONFIDENCE=$(printf "%s" "$PATTERN" | jq -r '.confidence // "high"')
CREATED_AT=$(printf "%s" "$PATTERN" | jq -r '.created_at // "unknown"')
RERUN_RATE=$(printf "%s" "$PATTERN" | jq -r '.rerun_success_rate // "N/A"')
